...
class Comlink(object):
    ...
    def __init__(self, channels=None, metadata=None, **kwargs):
            ...
            for channel in channels:
                ...
                try:
                    ...
                    raise AttributeError(
                        "`channels` must behave like a " "ComlinkChannel object"
                    )
        else:
            raise AttributeError(
                "`channels` is %s must be either a "
                "ComlinkChannel or a list of ComlinkChannels" % type(channels)
            )
        ...
        if channels is not None:
            if (
                ...
                warnings.warn(
                    "All supplied channel metadata (e.g. f_GHz) "
        ...
        self.metadata = {
            "site_a_latitude": metadata["site_a_latitude"],
        ...
        if "length" in list(metadata.keys()):
            ...
            if abs(length_diff) > 0.5:
                warnings.warn(
                    "Calculated length = %2.2f and supplied length "
    ...
    def __getattr__(self, item):
        ...
        if (item.split("_")[0] == "channel") and (type(int(item.split("_")[1])) == int):
    ...
    def calc_length(self):
        ...
        d_km = distance((coords.lat_a, coords.lon_a), (coords.lat_b, coords.lon_b))
        return d_km
    ...
    def plot_map(self, tiles="OpenStreetMap", fol_map=None):
        ...
        if fol_map is None:
            fol_map = folium.Map(
                location=[
                    (coords.lat_a + coords.lat_b) / 2,
                    (coords.lon_a + coords.lon_b) / 2,
                ],
                tiles=tiles,
                zoom_start=11,
            )
        fol_map.add_children(
            folium.PolyLine(
                [(coords.lat_a, coords.lon_a), (coords.lat_b, coords.lon_b)]
            )
        )
        return fol_map
    ...
    def plot_line(self, ax=None, *args, **kwargs):
        ...
        coords = self.get_coordinates()
        ax.plot(
            [coords.lon_a, coords.lon_b], [coords.lat_a, coords.lat_b], *args, **kwargs
        )
        return ax
    ...
    def plot_data(
        ...
        if ax is None:
            fig, ax = plt.subplots(
                len(columns), 1, figsize=(10, 1.5 * len(columns) + 1), sharex=True
            )
        try:
            ax[0].get_alpha()
        except TypeError:
        ...
        if channels is None:
            ...
            channels_to_plot = {ch_key: self.channels[ch_key] for ch_key in channels}
    ...
    def append_data(self, cml, max_length=None, max_age=None):
        ...
        for key in self.metadata.keys():
            if self.metadata[key] != cml.metadata[key]:
                raise ValueError(
                    "Comlink metadata `%s` is different"
        ...
        for ch_name in self.channels.keys():
            self.channels[ch_name].append_data(
                cml_ch=cml.channels[ch_name], max_length=max_length, max_age=max_age
            )
