...
cml_ch_metadata_dict = {
    "frequency": {"mandatory": True, "type": float, "min": 0.1e9, "max": 100e9},
    "polarization": {"mandatory": True, "type": str, "options": ["H", "V", "h", "v"]},
    "channel_id": {"mandatory": True, "type": str},
    "atpc": {"mandatory": False, "type": str, "options": ["on", "off"]},
}
...
cml_ch_data_names_dict_tx_rx = {
    "rx": {"mandatory": False, "quantity": "Received signal level", "units": "dBm"},
    "tx": {"mandatory": False, "quantity": "Trasmitted signal level", "units": "dBm"},
...
def write_to_cmlh5(
            ...
            if type(product_names) == str:
                if not strings_are_supplied:
                    raise AttributeError(
                        "`product_keys` was supplied as list,"
    ...
    if (t_start is None) and (t_stop is None):
        t_start, t_stop = _get_first_and_last_timestamp_in_cml_list(cml_list)

    if split_to_multiple_files:
        t_in_file_start_list = pd.date_range(
            start=t_start, end=t_stop, freq=splitting_period, normalize=True
        )
    ...
    # Write to file(s)
    for i, (t_in_file_start, t_in_file_stop) in enumerate(
        zip(t_in_file_start_list, t_in_file_stop_list)
    ):
        ...
        if split_to_multiple_files:
            try:
                ...
                raise ValueError(
                    "file name must contain a `.`, " "e.g. `my_cml_file.h5`"
                )
        ...
        with h5py.File(fn_i, mode="w") as h5file:
            ...
            for i_cml, cml in enumerate(cml_list):
                ...
                if product_keys is not None:
                    for i_prod, (product_key, product_name, product_unit) in enumerate(
                        zip(product_keys, product_names, product_units)
                    ):
...
def _get_first_and_last_timestamp_in_cml_list(cml_list):
    ...
    t_min = min(
        [
            min([cml_ch.data.index.min() for cml_ch in cml.channels.values()])
            for cml in cml_list
        ]
    )
    t_max = max(
        [
            max([cml_ch.data.index.max() for cml_ch in cml.channels.values()])
            for cml in cml_list
        ]
    )
    return t_min, t_max
...
def _write_channel_data(
    ...
    for column_name in cml_ch.data.columns:
        try:
            _cml_ch_data_names_dict[column_name] = cml_ch_data_names_dict_tx_rx[
                column_name
            ]
        except KeyError:
            pass
        try:
            _cml_ch_data_names_dict[column_name] = cml_ch_data_names_dict_tx_rx_min_max[
                column_name
            ]
        except KeyError:
            pass
    ...
    # Or add additional columns according to the info passed as argument
        ...
        if isinstance(column_names_to_write, dict):
            ...
            raise AttributeError(
                "`column_names_to_write` must be either a list" "or a dict"
            )
    else:
        raise AttributeError(
            "`write_all_data cannot be True if "
...
def _write_product(
    prod_g, cml, product_key, product_name, product_unit, compression, compression_opts
):
    """
    ...
    # Transform the pandas (np.datetime64) which is in ns to seconds
    t_vec = ts_t.astype("int64"), 1e9
    prod_g.create_dataset(
        "time", data=t_vec, compression=compression, compression_opts=compression_opts
    )
...
def _missing_attribute(attr_type):
    if attr_type == float:
        ...
        raise AttributeError(
            "Could not infer `missing_value` for " "`attr_type` %s" % attr_type
        )
    return fill_value
...
def read_from_multiple_cmlh5(
    fn_list, cml_id_list=None, t_start=None, t_stop=None, sort_fn_list=True
):
    """
    ...
    if (t_start is not None) and (t_stop is not None):
        ...
        for fn in fn_list:
            with h5py.File(fn, mode="r") as h5_reader:
                ...
                if (time_coverage_start < t_stop) and (time_coverage_stop > t_start):
    ...
    # If no start and stop data has been provided, just use fn_list
        ...
        raise ValueError(
            "`t_start` and `t_stop` must both be either `None` "
    ...
    for fn in fn_list_selected:
        cml_lists.append(
            read_from_cmlh5(
                fn=fn, cml_id_list=cml_id_list, t_start=t_start, t_stop=t_stop
            )
        )
...
def _read_cml_channel_data(
    cml_ch_g, t_start=None, t_stop=None, column_names_to_read=None, read_all_data=None
):
    """
    ...
    if (read_all_data is False) and (column_names_to_read is None):
        ...
        raise AttributeError(
            "`read_all_data cannot be True if "
    ...
    if len(list(data_dict.keys())) == 0:
        print(
            "Warning: No dataset matching the column names %s "
