...
def write_to_wasim_input_file(cml_list,
                                       fn,
    ...
    for cml in cml_list:
        df[cml.metadata['cml_id']] = (cml.channels[channel_name]
                                      .data.R
    ...
    cols = cols_time + [col_name for col_name in df.columns
                        if col_name not in cols_time]
    df = df[cols]
    ...
    for cml in cml_list:
        ...
        elif ((source_projection is not None)
