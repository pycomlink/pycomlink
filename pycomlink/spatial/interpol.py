...
@deprecated(
    "Use the new `Interpolator` classes from " "`pycomlink.spatial.interpolator`."
)
class Interpolator(object):
    ...
    def __init__(
        ...
        if (xgrid is None) or (ygrid is None):
            ...
            xcoords = np.arange(
                min(lons) - resolution, max(lons) + resolution, resolution
            )
            ycoords = np.arange(
                min(lats) - resolution, max(lats) + resolution, resolution
            )
        ...
        self.lons = np.array([cml.get_center_lon_lat()[0] for cml in self.cml_list])
        self.lats = np.array([cml.get_center_lon_lat()[1] for cml in self.cml_list])
    ...
    def idw_kdtree(
        self, nnear=10, p=2, eps=0.1, progress_bar=False, t_start=None, t_stop=None
    ):
        ...
        for t, row in self.df_cmls_R[t_start:t_stop].iterrows():
            ...
            idw_tree = Invdisttree(
                X=np.array([self.lons[i_not_nan], self.lats[i_not_nan]]).T,
                leafsize=nnear + 2,
            )
            interp_values = idw_tree(
                q=np.array([self.xgrid.flatten(), self.ygrid.flatten()]).T,
                z=values[i_not_nan],
                nnear=nnear,
                p=p,
                eps=eps,
    ...
    def _fields_to_dataset(self, fields, t_start=None, t_stop=None):
        ...
        ds = xr.Dataset(
            {self.variable: (["x", "y", "time"], np.moveaxis(np.array(fields), 0, -1))},
            coords={
                "lon": (["x", "y"], self.xgrid),
                "lat": (["x", "y"], self.ygrid),
                "time": (
                    self.df_cmls_R[t_start:t_stop].index.values.astype(np.datetime64)
                ),
            },
        )
        return ds
