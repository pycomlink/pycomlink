...
class TestRoundTripCmlh5(unittest.TestCase):
    ...
    def testWithOneFileOneCml(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            for ch_name in cml.channels.keys():
                assert (
                    cml.channels[ch_name].metadata["frequency"]
                    == cml_loaded.channels[ch_name].metadata["frequency"]
                )
                pd.util.testing.assert_frame_equal(
                    cml.channels[ch_name].data, cml_loaded.channels[ch_name].data
                )
    ...
    def testWithMultiFilesTwoCmls(self):
        ...
        with TemporaryDirectory() as temp_dir:
            temp_fn = os.path.join(temp_dir, "test_cmlh5.h5")
            pycml.io.write_to_cmlh5(
                cml_list, fn=temp_fn, splitting_period="D", split_to_multiple_files=True
            )

            temp_fn_list = glob(temp_fn.split(".")[0] + "*")
            assert len(temp_fn_list) == len(
                cml_list[0].channel_1.data.resample("D").mean().index
            )
