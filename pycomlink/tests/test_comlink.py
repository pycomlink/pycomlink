...
class TestComlinkCopy(unittest.TestCase):
    def test_copy(self):
        ...
        assert cml_copy.metadata["site_a_latitude"] == cml.metadata["site_a_latitude"]
...
class TestComlinkChannelAppendData(unittest.TestCase):
    ...
    def test_append_wrong_cml(self):
        ...
        self.assertRaises(ValueError, cml.append_data, cml_wrong)


def generate_standard_cml():
    cml = Comlink(
        channels=deepcopy(cml_ch),
        metadata={
            "site_a_latitude": 44.1,
...
def assert_comlink_equal(cml_1, cml_2):
    ...
    for ch_name in cml_1.channels.keys():
        assert_comlink_channel_equal(cml_1.channels[ch_name], cml_2.channels[ch_name])
