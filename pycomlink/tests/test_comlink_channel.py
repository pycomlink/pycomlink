...
class TestComlinkChannelInit(unittest.TestCase):
    def test_with_DataFrame_rx_and_tx(self):
        ...
        pd.util.testing.assert_almost_equal(
            cml_ch.index, pd.DatetimeIndex(t_date_range, name="time")
        )
...
class TestComlinkChannelAppendData(unittest.TestCase):
    ...
    def test_append_wrong_cml_ch(self):
        df = pd.DataFrame(index=t_date_range, data={"rx": rx_list, "tx": tx_list})
        cml_ch_shortened = ComlinkChannel(data=df.iloc[:100, :], frequency=f)
        cml_ch_rest_of_data = ComlinkChannel(
            data=df.iloc[100:, :], frequency=22.235 * 1e9
        )

        self.assertRaises(ValueError, cml_ch_shortened.append_data, cml_ch_rest_of_data)
...
def assert_comlink_channel_equal(cml_ch_1, cml_ch_2):
    ...
    for key in cml_ch_1.__dict__:
        if key == "data":
            pd.util.testing.assert_frame_equal(cml_ch_1.data, cml_ch_2.data)
        else:
            assert cml_ch_1.__dict__[key] == cml_ch_2.__dict__[key]
