...
class TestWetDryStdDev(unittest.TestCase):
    def test_standrad_processing(self):
        ...
        assert cml.channel_1.data.wet.loc["2016-11-02 13:00"].values[0] == False

        assert cml.channel_2.data.wet.loc["2016-11-02 14:00"].values[0] == True
        assert cml.channel_2.data.wet.loc["2016-11-02 13:00"].values[0] == False

        cml.process.wet_dry.std_dev(window_length=30, threshold=0.8)

        assert cml.channel_1.data.wet.loc["2016-11-02 14:00"].values[0] == False
        assert cml.channel_1.data.wet.loc["2016-11-02 13:00"].values[0] == False
        assert cml.channel_1.data.wet.loc["2016-11-02 14:30"].values[0] == True

        assert cml.channel_2.data.wet.loc["2016-11-02 14:00"].values[0] == False
        assert cml.channel_2.data.wet.loc["2016-11-02 13:00"].values[0] == False
        assert cml.channel_2.data.wet.loc["2016-11-02 14:30"].values[0] == True
        ...
        R_from_function = pycml.processing.A_R_relation.A_R_relation.calc_R_from_A(
    ...
    def test_processing_for_selected_time_period(self):
        ...
        cml.process.wet_dry.std_dev(window_length=60, threshold=0.8, t_start=t_start)
        assert cml.channel_1.data.wet.loc["2016-11-02 18:30"].values[0] == True
        assert cml.channel_1.data.wet.loc["2016-11-02 14:00"].values[0] == False
        assert cml.channel_1.data.wet.loc["2016-11-02 13:00"].values[0] == False
        ...
        cml.channel_1.data.wet = False
        cml.process.wet_dry.std_dev(window_length=60, threshold=0.8, t_stop=t_stop)
        assert cml.channel_1.data.wet.loc["2016-11-02 18:30"].values[0] == False
        assert cml.channel_1.data.wet.loc["2016-11-02 14:00"].values[0] == True
        assert cml.channel_1.data.wet.loc["2016-11-02 13:00"].values[0] == False
