class Validator(object):
    ...
    def calc_stats(self, cml, time_series):
        cml_copy = cml
        time_series_df = pd.DataFrame(data = {'xr_ds': time_series}, index = time_series.time)
...
class GridValidator(Validator):
    def __init__(self, lats=None, lons=None, values=None, xr_ds=None):
        if xr_ds is None:
            xr_ds = xr.Dataset() # construct xr_ds from lats, lons & values here?
    ...
    def get_time_series(self, cml, values):
        ...
        for i in range(len(intersect)): 
            for j in range(len(intersect)):
                if intersect[i][j] != 0:
                    grid_sum = grid_sum + (intersect[i][j] * self.xr_ds[values][:,i,j])
...
def calc_intersect_weights(cml, xr_ds, offset=None):
    ...
    ur_cell = grid[(len(grid)-1),(len(grid)-1),0] - grid[(len(grid)-1), (len(grid)-2), 0]
    ...
    lon_grid = grid[:,:,0]
    lat_grid = grid[:,:,1]
    bounding_box = (((lon_grid > lon_min-offset) & (lon_grid < lon_max+offset)) & 
